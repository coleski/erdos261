...
r_a=2^{a-1}\left(\frac{n}{2^n}-
  \sum_{j\in A\cap\lbrace n+1,\ldots,a-1\rbrace}\frac{j}{2^j}\right).
$$
...
$\varepsilon_a\in\lbrace 0,1\rbrace$ gives the exact transition
...
$a$, beginning with $R_{n+1}=\lbrace n\rbrace$. Then:
...
The claim holds initially because $R_{n+1}=\lbrace n\rbrace$. Suppose it holds at
index $a$.

From a singleton $\lbrace r\rbrace$, both digit choices are admissible exactly when
...
Now suppose $R_a=\lbrace r,(a+1)-r\rbrace$, ordering the pair so that
...
$$
\frac{16}{2^{16}}=
\sum_{a\in\lbrace 17,18,19,22,23,24,28,32,33,34,35,36,39,42,43,45,46\rbrace}
\frac{a}{2^a}.
$$
